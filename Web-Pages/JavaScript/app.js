...
const pool = mysql.createPool({
  ...
  user: 'root',
  password: 'shivadhar',
  database: 'rohitLakshmiDeepikadatabase',
...
app.post('/saveuser', async (req, res) => {
  ...
  try {
    ...
    if (result.length > 0 && result[0][0].Result === 'Success') {
      res.json({ message: 'Data saved successfully' });
    } else {
      res.status(400).json({ error: result[0][0].Result });
    }
    console.log('Stored procedure called successfully');
    console.log(result);
  } catch (err) {
...
app.post('/addPropertyRating', async (req, res) => {
  const { rating, review, propertyId, customerId } = req.body;
 
  let connection;
  try {
    connection = await pool.getConnection(); // Get a connection from the pool

    const query = `CALL AddPropertyRating(?, ?, ?, ?) `;
    const [result] = await connection.query(query, [rating, review, propertyId, customerId]);
...
app.get('/GetPropertyRatings/:propertyId', async (req, res) => {
  const p_PropertyId = req.params.propertyId;

  let connection;
  try {
    connection = await pool.getConnection();
    
    const sql = 'CALL GetPropertyRatings(?)';
    const [rows] = await connection.query(sql, [p_PropertyId]);
...
app.delete('/DeleteSavedProperty/:propertyId', async (req, res) => {
  const p_PropertyId = req.params.propertyId;
  ...
  try {
    ...
    const sql = 'CALL DeleteSavedProperty(?)';
    const [result] = await connection.query(sql, [p_PropertyId]);
    
    connection.release();
    if (result.length > 0 && result[0][0].Result === 'Success') {
      res.json({ message: 'Property deleted successfully' });
    } else {
      res.status(400).json({ error: 'Property not deleted. Please try again!' });
    }
  } catch (err) {
    console.error('Error calling stored procedure:', err);
    res.status(500).json({ error: 'An error occurred' });
...
app.post('/savedProperties', async (req, res) => {
  const { propertyId, customerId} = req.body;
  ...
  try {
    ...
    const query = `CALL InsertSavedProperty(?, ?) `;
    const [result] = await connection.query(query, [propertyId, customerId]);
...
app.post('/getSavedProperties', async (req, res) => {
